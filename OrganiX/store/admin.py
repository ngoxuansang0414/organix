...
class OrderAdmin(admin.ModelAdmin):
    ...
    def analytics(self, request):
        ...
        order_status_percentage_data = list(Order.objects.values_list('status').annotate(percentage = 100*Count('status')/float(order_count)).order_by('-percentage'))
        ...
        for i in range(60, -30 , -30):
            ...
            completed_order = Order.objects.filter(Q(created_at__year = year_time) & Q(created_at__month = month_time), status = 'Hoàn thành')
            revenue = completed_order.aggregate(revenue = Sum('total_price'))
            profit = completed_order.aggregate(profit = Sum(F('orderitem__quantity')*F('orderitem__product__sale_price')-F('orderitem__quantity')*F('orderitem__product__original_price')))
        ...
        print(cost_list)
        return render(request, 'analytics/orders.html', {'status': status, 'percentage': status_percentage, 'profit_list': profit_list, 'cost_list': cost_list, 'month_list': month_list, 'title': 'Thống kê đơn hàng'})
...
class ProductAdmin(admin.ModelAdmin):
    list_display = 	['name', 'original_price', 'sale_price', 'category', 'created_at', 'modified_at']
    ...
    def analytics(self, request):
        ...
        hot_product = list(
            hot_product.values_list("name").annotate(product_selled=Sum('orderitem__quantity')).order_by('-product_selled')
        )
        products = [x[0] for x in hot_product[:5]]
        data = [x[1] for x in hot_product[:5]]
        return render(request, 'analytics/products.html', {'products': products, 'data': data, 'title': 'Sản phẩm bán chạy'})
...
class ReviewRatingAdmin(admin.ModelAdmin):
    ...
    def analytics(self, request):
        ...
        return render(request, 'analytics/rating.html', {'rating_data': rating_data, 'title': 'Thống kê tỉ lệ đánh giá'})
    def get_urls(self):
        urls = super().get_urls()
        from django.urls import path
        custom_urls = [ path("analytics/", self.analytics), ]
